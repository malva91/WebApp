...
try {
	...
	rs = st.executeQuery("select * from tags where attivo > 0 order by tipo,luogo,id"); 
		
	while(rs.next()){
		if(tipo.contains(rs.getString("tipo"))){
			risp += rs.getString("luogo")  + "\t" + rs.getString("id")  + "\t" + rs.getString("colore") + "\t" + rs.getString("tipo") + "\n";
		}
	}
} catch (Exception e) {
	risp = "---- ERRORE ---- " + e;
